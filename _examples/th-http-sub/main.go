package main

import (
	"bufio"
	"crypto/rsa"
	"crypto/x509"
	"encoding/json"
	"encoding/pem"
	"fmt"
	"github.com/telehash/gogotelehash"
	"github.com/telehash/gogotelehash/net"
	"github.com/telehash/gogotelehash/net/ipv4"
	"github.com/telehash/gogotelehash/net/ipv6"
	"io"
	"net/http"
	"runtime"
	"strings"
)

func main() {
	runtime.GOMAXPROCS(runtime.NumCPU() * 2)

	sw := telehash.Switch{
		Transports: []net.Transport{
			&ipv4.Transport{Addr: ":0"},
			&ipv6.Transport{Addr: ":0"},
		},
	}

	must(sw.Start())
	defer sw.Stop()

	parse_main_seed(&sw)

	http.ListenAndServe(":3000", &proxy{&sw, "dev"})
}

func must(err error) {
	if err != nil {
		panic(err)
	}
}

type proxy struct {
	sw     *telehash.Switch
	domain string
}

func (r *proxy) ServeHTTP(rw http.ResponseWriter, req *http.Request) {
	var (
		host string
	)

	if host == "" {
		host = req.Host
	}
	if host == "" {
		host = req.URL.Host
	}
	if host == "" {
		host = req.Header.Get("Host")
	}
	if host == "" {
		http.Error(rw, "Ooops 0001", 502)
		return
	}

	host = strings.TrimSuffix(host, ".")
	host = strings.TrimSuffix(host, "."+r.domain)
	parts := strings.Split(host, ".")

	telehash.Log.Errorf("host=%s", host)

	target, err := telehash.HashnameFromString(parts[len(parts)-1])
	if err != nil {
		http.Error(rw, "Ooops 0002", 502)
		return
	}

	host = strings.Join(parts[:len(parts)-1], ".")
	req.Host = host
	req.URL.Host = host
	req.Header.Set("Host", host)

	if r.sw.Seek(target).IsZero() {
		http.Error(rw, "Ooops 0006", 502)
		return
	}

	telehash.Log.Errorf("target=%s", target)

	channel, err := r.sw.Open(telehash.ChannelOptions{Type: "_fd/http", To: target})
	if err != nil {
		http.Error(rw, "Ooops 0003", 502)
		return
	}
	defer channel.Close()

	bw := bufio.NewWriterSize(channel, 1200)
	err = req.Write(bw)
	if err != nil {
		http.Error(rw, "Ooops 0004", 502)
		return
	}
	err = bw.Flush()
	if err != nil {
		http.Error(rw, "Ooops 0007", 502)
		return
	}

	res, err := http.ReadResponse(bufio.NewReaderSize(channel, 1500), req)
	if err != nil {
		http.Error(rw, "Ooops 0005", 502)
		return
	}
	defer res.Body.Close()

	for h, v := range res.Header {
		rw.Header()[h] = v
	}
	rw.WriteHeader(res.StatusCode)

	io.Copy(rw, res.Body)
}

const seed = `
[
{
  "ip": "208.68.164.253",
  "port": 42424,
  "ip6": "2605:da00:5222:5269:230:48ff:fe35:6572",
  "port6": 42424,
  "hashname": "5fa6f146d784c9ae6f6d762fbc56761d472f3d097dfba3915c890eec9b79a088",
  "pubkey": "-----BEGIN PUBLIC KEY-----\\nXwIDAQAB\n-----END PUBLIC KEY-----\n",
  "http": "http://208.68.164.253:42424",
  "bridge": true
}, {
  "ip": "173.255.220.185",
  "port": 42424,
  "ip6": "2600:3c01::f03c:91ff:fe70:ff59",
  "port6": 42424,
  "hashname": "b61120844c809260126aa0cf75390ef7f72c65a9ce03366efcf89ff549233758",
  "pubkey": "-----BEGIN PUBLIC KEY-----\\nuQIDAQAB\n-----END PUBLIC KEY-----\n"
}, {
  "ip": "204.45.252.101",
  "port": 42424,
  "hashname": "6b171cedc8945ca7ba078392c0d1bc34fe0e7f161fc60e7b1cdb246f68bcb683",
  "pubkey": "-----BEGIN PUBLIC KEY-----\\nPwIDAQAB\n-----END PUBLIC KEY-----\n"
}, {
  "ip": "208.126.199.195",
  "port": 42424,
  "ip6": "2001:470:c0a6:3::10",
  "port6": 42424,
  "hashname": "39c7f1d641947f51960ec5ab070680ea9dff110e8406cb07e4ae093a2e5d823a",
  "pubkey": "-----BEGIN PUBLIC KEY-----\\nLwIDAQAB\n-----END PUBLIC KEY-----\n",
  "http": "http://208.126.199.195:42424",
  "bridge": true
}, {
  "ip": "162.243.1.152",
  "port": 42424,
  "hashname": "9ba9c175c3c26af9df5c8163ea91d4ae4eca59ba95d66deb287c89ea0c596979",
  "pubkey": "-----BEGIN PUBLIC KEY-----\\nEQIDAQAB\n-----END PUBLIC KEY-----\n"
},
  {
    "IP": "95.85.6.236",
    "Port": 42424,
    "Hashname": "f3a2d1ff11f67069feac11bc562c32549e30135f56a9a9c6575499d6a7c72915",
    "Pubkey": "-----BEGIN PUBLIC KEY-----\\ntQIDAQAB\n-----END PUBLIC KEY-----\n"
  }
]
`

func parse_main_seed(s *telehash.Switch) {
	type seed_t struct {
		IP       string
		Port     int
		Hashname string
		Pubkey   string
	}
	var (
		seeds     []seed_t
		pem_block *pem.Block
	)

	err := json.Unmarshal([]byte(seed), &seeds)
	if err != nil {
		panic(err)
	}

	for _, seed := range seeds {
		go func(seed seed_t) {
			pem_block, _ = pem.Decode([]byte(seed.Pubkey))

			if pem_block.Type != "PUBLIC KEY" {
				return
			}

			addr, err := ipv4.ResolveAddr(fmt.Sprintf("%s:%d", seed.IP, seed.Port))
			if err != nil {
				fmt.Printf("failed to seed: %s\n  %s\n", err, addr)
				return
			}

			keyi, err := x509.ParsePKIXPublicKey(pem_block.Bytes)
			if err != nil {
				fmt.Printf("failed to seed: %s\n  %s\n", err, addr)
				return
			}

			key, ok := keyi.(*rsa.PublicKey)
			if key == nil {
				fmt.Printf("failed to seed: %s\n  %s\n", "not an rsa key", addr)
				return
			}
			if !ok {
				fmt.Printf("failed to seed: %s\n  %s\n", "not an rsa key", addr)
				return
			}

			hn, err := s.Seed("ipv4", addr, key)

			if err != nil {
				fmt.Printf("failed to seed: %s\n  %s\n", err, addr)
				return
			}

			fmt.Printf("connected to %s\n", hn.Short())
		}(seed)
	}
}
